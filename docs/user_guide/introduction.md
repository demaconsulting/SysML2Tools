...
non-recursive. Confirmed by hand-rendering this exact fixture: the tree contains only the
`myVehicle` row — neither `Vehicle`'s own `engine`/`wheel` parts, nor `Engine`'s `cylinder`, are
included, because exact-match scoping does not pull in either exposed name's descendants.
