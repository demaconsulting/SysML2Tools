...
subtree (the common "expose an inner state/action/part/lifeline of the root" case). Used only by
the four single-root strategies to restrict their heuristic root selection to one relevant to the
resolved scope.

##### Error Handling

N/A — none of the three methods validate arguments or throw; a `null` `viewNode` and an empty or
...
- `GeneralViewLayoutStrategy` calls all three methods (moved here verbatim from its own former
  private copies of `ResolveExposedScope` and `IsInSubjectScope`).
- `GridViewLayoutStrategy` and `BrowserViewLayoutStrategy` call `ResolveExposedScope` and
  ...
  single-root heuristic, so `IsRootRelevantToScope` does not apply).
- `InterconnectionViewLayoutStrategy`, `StateTransitionViewLayoutStrategy`,
  `ActionFlowViewLayoutStrategy`, and `SequenceViewLayoutStrategy` call `ResolveExposedScope` to
  compute the scope, `IsRootRelevantToScope` to restrict their heuristic root selection to a root
  relevant to that scope, and `IsInSubjectScope` to filter the child elements (parts, states,
  actions, lifelines) collected from the selected root.
