...
supplies reusable, independently testable helpers consumed by other subsystems. Its sole
responsibility is safe file-path manipulation (protecting callers from path-traversal
vulnerabilities when constructing paths from caller-supplied inputs). The `Utilities`
subsystem contains one unit: `PathHelpers`.

> Qualified-name compaction for Markdown-oriented rendering (`QualifiedNameShortener`) moved
> to the `DemaConsulting.SysML2Tools.Core` library's `Query` subsystem alongside the rest of
> the public query API — see `docs/design/sysml2-tools-core/query.md`.
...
The `Utilities` subsystem contains the `PathHelpers` unit. It does not depend on any other
tool unit or subsystem; it uses only .NET BCL types (`Path`/`ArgumentNullException`).
